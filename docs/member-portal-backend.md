...
QQ SMTP and both Confirm Signup/Magic Link templates using `{{ .Token }}` are saved. The user's confirmation that the code was copied is evidence of email receipt, but not successful OTP verification or administrator login. No SMTP password, authorization code or login code is stored in this document.
