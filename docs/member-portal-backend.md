...
All 4 cloud migrations succeeded, the signup Hook is enabled, QQ SMTP and both `{{ .Token }}` templates are saved, and live portal configuration is `enabled: true`. Review-notification worker deployment, SMTP authentication and the active once-per-minute schedule are verified. One real reminder has been accepted by SMTP and two are queued for network-related retry; mailbox receipt is not claimed. Real member first login, draft save, upload, submit/review and nonempty publication still need verification during member use. Local fixture/SQL tests are not evidence that those cloud user flows passed. See the concise [member/admin guide](member-portal-guide.md).
