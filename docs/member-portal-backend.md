...
Implemented by `supabase/migrations/202609190001_member_portal.sql`, `202609190002_signup_allowlist.sql`, `202609190003_encoded_email_controls.sql`, and `202609200004_review_notifications.sql`. This document describes actual SQL behavior and activation steps; SQL installation and verified Auth Hook activation are separate states.

Current status (2026-09-20): the live roster contains 36 members and 129 public content pages. There are 37 enabled access records: 35 ordinary members and 2 administrators, one assigned member and one independent administrator. 郭硕 (`member-37`) is live as a clinical research member with registered email `guoshuo@wchscu.cn`; Akhil Venugopal (`member-36`) retains the user-designated Outlook login address. Allowlist changes do not create Auth users or send invitation emails; members request their own codes and edit/submit their own content without PI proxy operation. Real administrator OTP login is verified. The actual publish button called Edge and triggered [run 35478645703](https://github.com/jojolee0731/tian-lab-site/actions/runs/35478645703), with successful build and deploy. The approved public profile catalog remains empty.

All 4 cloud migrations succeeded, the signup Hook is enabled, QQ SMTP and both `{{ .Token }}` templates are saved, and live portal configuration is `enabled: true`. Review-notification worker deployment, SMTP authentication and the active once-per-minute schedule are verified. Real reminders have been accepted by SMTP, including one delivered by the scheduled retry; mailbox receipt is not claimed. Three real pending submissions (胡潘宜、陶怡然、赵芮) are visible in the cloud administrator UI. Individual login/edit/upload paths were not reproduced; review and nonempty publication still need verification during member use. Local fixture/SQL tests are not evidence that those cloud user flows passed. See the concise [member/admin guide](member-portal-guide.md).
...
3. Configure Supabase Auth for email OTP sign-in and the exact allowed site/redirect URLs. QQ SMTP and both Token templates are saved, and real administrator sign-in succeeded on 2026-09-20. All 36 current members have explicit enabled allowlist entries, including `member-36` and `member-37`; members request their own login codes. Keep the SMTP authorization code in service configuration only. Only a confirmed Auth email matching the live allowlist gains portal access; future public roster changes do not authorize access automatically.
...
Migration 004 installs a private outbox tied transactionally to new submission snapshots. Draft saves do not notify. One submission has one outbox entry; saving unchanged content and submitting again while the latest snapshot awaits review does not create another reminder. This feature does not email all members or create Auth accounts. The fixed recipient is 陈香淼 `xiangm_chen@foxmail.com`; the claim RPC checks that this address is still an enabled administrator, the member is active and the submission still awaits review. It never redirects to another administrator automatically.

Cloud evidence on 2026-09-20: migration 004 and `supabase/functions/review-notifications` are deployed; `REVIEW_SMTP_PASSWORD` has been saved by the user. Anonymous and public-key requests returned HTTP401. A server-authenticated `{ "mode": "verify" }` request returned HTTP200 with `smtp_verified`; this checks SMTP connection/authentication without claiming queued work or sending mail. The once-per-minute job is active. Database-origin verification failed in automatically selected Mumbai but passed when pinned to Tokyo (`ap-northeast-1`), so the scheduler sends the official `x-region` header. Real queue checks confirmed SMTP acceptance and a subsequent scheduled retry succeeding. Check the administrator status for current pending/failed counts. Actual mailbox receipt remains unverified.

The worker accepts only server POST calls carrying an exact known elevated key in `apikey`. It supports the Supabase-managed `SUPABASE_SECRET_KEYS` map and secret/service-role fallback; modern secret keys must not be used as Bearer JWTs. Deploy with platform JWT verification disabled for this function (`--no-verify-jwt`), because its handler performs this separate service-key check; anonymous, public-key and user-JWT callers remain refused. `publish-site` continues to require a real user JWT and live admin role. No worker body can select a recipient, custom message or batch size.

The scheduler should call this fixed function every minute with `{}` and an existing project server key read from private Vault configuration. Do not embed credentials in SQL committed to Git or in public site files. Each invocation claims at most 5 entries through `claim_review_notifications({p_limit:5})`; each entry contains only `id`, `submission_id`, `member_name`, `created_at`, `lease_token` and `attempts`. The worker sends via QQ SMTP (`smtp.qq.com:465`, TLS), from `TianLab 成员工作台 <71529983@qq.com>`, using only the server secret `REVIEW_SMTP_PASSWORD`. Each message includes sanitized member name, submission time and the fixed [review entry](https://jojolee0731.github.io/tian-lab-site/members/); no draft, biography, project text or image is mailed.

Delivery has an 18-second absolute SMTP deadline with socket closure, a 5-minute claim lease and a new token per claim. `finish_review_notification` requires the same unexpired token. Failures back off 1/5/15/60 minutes and stop after 8 automatic attempts. Review or superseding submission causes obsolete work to be skipped. `review_notification_status()` returns admin-only counts and last successful SMTP completion time; it does not establish mailbox receipt. `retry_review_notification(p_id)` is service-only recovery for a failed item after its cause is resolved. These RPCs do not imply a separate retry/status UI already exists.

Stable submission-based Message-ID reduces duplication but cannot ensure exactly-once delivery. After SMTP acceptance followed by a failed finish RPC, the invocation does not send that item again; a later expired-lease retry can duplicate it. Operational checks should separately confirm scheduler activation, queue processing, SMTP acceptance and actual recipient receipt. Reminder failure never removes a valid submission or automatically approves/publishes content; members can submit themselves and administrators can review directly in the portal.
...
The runner creates a temporary native PostgreSQL cluster, listens on a private Unix socket only, applies the migrations and runs the permission and notification SQL suites. It stops the server and deletes the test cluster in `finally`. It creates no operating-system service. `fixture.sql` supplies minimal `auth.users`, `auth.uid()`, Storage tables and Supabase role names; it must never be applied to a real Supabase project.

The historical 2026-09-19 combined run passed 159 assertions on native PostgreSQL 16.15. The 2026-09-20 suite including 004 passed 222 assertions plus a real two-connection concurrent-claim test. The final worker separately passed 29 Node tests, including service-key authorization, fixed content, absolute timeout/late TLS cancellation and SMTP-accepted/finish-failed handling. These results are reused; no tests were rerun for this documentation update. Tests execute as real database roles, not a mocked JavaScript RLS layer. Covered behavior includes unauthorized/disabled/unverified sessions, account-bound edits, stale revisions, immutable submissions/reviews, unauthorized approval/export/table reads, own review feedback, path and field validation, private uploads, overwrite/delete resistance even in the presence of broad Storage policies, approval export versus newer private drafts, restore history/version increments, signup allowlist checks, encoded email controls, and removal of inactive members from export.

This is an actual PostgreSQL RPC/privilege/RLS execution result. It does **not** verify hosted user sessions, real member Storage uploads or the member review workflow. Separate cloud evidence confirms login-code email receipt, code deployment and direct PAT dispatch; the remaining user-flow checks are listed above.
...
QQ SMTP and both Confirm Signup/Magic Link templates using `{{ .Token }}` are saved. Login-code email receipt was confirmed on 2026-09-19; real administrator OTP login was separately completed on 2026-09-20. No SMTP password, authorization code or login code is stored in this document.
...
The local suite now includes all 4 migrations and runs without cloud access:

```sh
python3 supabase/tests/run_local_sql.py
```

Historical result after 003: 159 assertions passed on native PostgreSQL 16.15 (the original 80, 13 hook assertions and 66 encoded-control checks); 004 expands this to 222 assertions and the concurrent-claim test described above. The earlier `member-36` site check covered 126 pages, 3168 local references and 35 members; the current deployed roster includes `member-37`, 36 members and 129 pages. Separate cloud evidence confirms administrator OTP login, enabled access for current members and the authenticated Edge publish button through successful deployment. Three real submissions have been observed in the administrator UI. Individual login/edit/upload paths, cross-member access, review and nonempty publication remain to be verified during use; enabling access is not evidence of that member's OTP login. Reminder scheduling is active; actual mailbox receipt remains unverified as stated above.

## Official technical references

- [Nodemailer SMTP transport](https://nodemailer.com/smtp): TLS and `verify()` behavior; connection/authentication verification does not send a message. The worker pins official npm package `nodemailer@10.0.10`.
...
- [Supabase regional invocation](https://supabase.com/docs/guides/functions/regional-invocation): `x-region` selects the tested SMTP egress region; pinned calls do not automatically fail over during a regional outage.
