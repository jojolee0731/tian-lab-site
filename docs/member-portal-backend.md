...
Current status (2026-09-20): real administrator OTP login is verified. The earlier 34-member batch was enabled through the live administrator UI; a reload confirmed each email-to-member-ID mapping. One assigned member was subsequently promoted to `admin` with explicit user authorization, retaining the original independent administrator. The newly added postdoctoral fellow Akhil Venugopal (`member-36`) was separately enabled as `member` using the user-designated Outlook login address. Saving and reloading confirmed the new enabled row and the current total of 34 ordinary members and 2 administrators, for 36 access records covering all 35 public members. His public contact email and login email both use Outlook, following the user's latest preference. Allowlist updates do not create Auth users or send invitation emails. The administrator clicked the actual publish button, which called Edge and triggered [run 35478645703](https://github.com/jojolee0731/tian-lab-site/actions/runs/35478645703); build and deploy both succeeded. The approved public profile catalog remains empty.
...
3. Configure Supabase Auth for email OTP sign-in and the exact allowed site/redirect URLs. QQ SMTP and both Token templates are saved, and real administrator sign-in succeeded on 2026-09-20. All 35 current members have explicit enabled allowlist entries, including the separately verified `member-36` entry; members request their own login codes. Keep the SMTP authorization code in service configuration only. Only a confirmed Auth email matching the live allowlist gains portal access; future public roster changes do not authorize access automatically.
...
After adding003, the combined suite passed 159 assertions on native PostgreSQL16.15: the original 80, 13 hook assertions, and 66 encoded-control checks. The unchanged SQL result is retained; SQL tests were not repeated for this documentation update. The 2026-09-20 local site check after adding `member-36` passed with 126 public content pages, 3168 local references and 35 members. Separate cloud evidence confirms administrator OTP login, the earlier 34-member allowlist batch, the separately saved and reloaded `member-36` mapping, and the authenticated Edge publish button through successful deployment. Real member first login, draft/upload/submit/review, cross-member access and nonempty content publication remain to be verified during use; enabling the new member is not evidence of that member's OTP login.
