...
成员工作台在独立 worktree `tian-lab-site-members-20260919`、分支 `codex/member-workbench-20260919` 中实现。2026-09-19 当前状态：3 项云端迁移已执行，邀请检查 Hook 已启用；QQ SMTP、Confirm Signup 与 Magic Link 的验证码模板均已保存，用户已确认收到并复制验证码。本地工作台 `enabled: true`；初始访问范围仅管理员，普通成员未批量邀请，管理员登录完成仍待验证。

单仓库 Actions 发布令牌已保存到 Supabase Edge Secret，GitHub Actions 的 Supabase URL/secret 也已保存。专用 GitHub 令牌于 **2027-09-19** 到期，届时需更换并更新 Edge Secret。配置与收信成功不等于管理员登录或整条发布流程已验证。
...
发布前完成实际服务配置及验证，并将 GitHub Pages 的构建来源设置为 GitHub Actions。新的 `.github/workflows/publish-members.yml` 支持两条路径：普通 `main` push 构建、检查并部署当前静态内容；管理员授权的 `workflow_dispatch` 先从 Supabase 拉取获批快照与私有图片，再构建、检查、限定公开文件提交和部署。具体 secrets 配置见后端说明，密钥不进入公开配置。
