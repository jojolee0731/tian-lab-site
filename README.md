...
科研内容优先的静态网站，包含英文、中文和西班牙语三种语言。每种语言有首页、研究方向、研究成果、团队、加入、学术合作和新闻归档七个页面；35 名成员各有三语个人主页，共 126 个公开内容页面。另有需要登录的成员工作台 `members/`，不列入公开站点地图。
...
截至2026-09-20，真实管理员已完成登录。此前通过正式网站管理员界面为34位成员开通权限，并在重新加载后核对每人的邮箱与本人ID；其中1位按用户明确授权兼任 `admin`，原独立管理员保留。本次另为新增博士后 Akhil Venugopal（`member-36`）绑定用户指定的 Outlook 登录邮箱，角色为 `member`；保存并重新加载后确认该记录已开通。当前35位公开成员均有本人编辑权限，共34位普通成员、2位管理员、36条访问记录。开通操作只维护访问名单，不创建 Auth 用户或发送邀请邮件；成员需使用已登记邮箱自行请求验证码登录，公开联系邮箱可与登录邮箱不同。
...
| `data/people.json` | PI 及 35 名现有成员、分组、真实肖像、角色、单位、研究方向和有来源的姓名；姓名来源边界见内容决策 |
...
| `scripts/build_site.py` / `scripts/member_profile.py` | 页面结构、三语 UI 和个人内容安全校验；生成 126 个公开 HTML 页面及 sitemap |
...
2026-09-20新增 `member-36` 后的本地检查通过：126个公开内容页面、3168个本地引用、24篇论文、35位现有成员。该结果验证本地生成产物，不替代部署结果或真实成员登录验证。
